<script lang="ts">
	...
	// Mock API endpoints
	const categories = ['Vehicles', 'Reservations', 'Charging', 'Users', 'Golf Courses', 'Models', 'Firmware'];
	...
	const endpoints: ApiEndpoint[] = [
		{
			...
			parameters: [
				{ name: 'status', type: 'string', required: false, description: 'Filter by vehicle status (operating, charging, idle, maintenance, error)' },
				{ name: 'golfCourseId', type: 'string', required: false, description: 'Filter by golf course ID' },
			...
			responses: [
				{
					...
					example: JSON.stringify({
						data: [
							{ id: 'vehicle-1', vin: 'VIN123', status: 'operating', batteryLevel: 85, model: 'Cart Pro X1' }
						],
			...
			examples: [
				{
					...
					request: 'curl -X GET "https://api.aprofleet.com/v1/vehicles?status=operating" \\\n  -H "Authorization: Bearer YOUR_TOKEN"',
					response: '{\n  "data": [...],\n  "pagination": {...}\n}'
				}
			]
		},
		...
		},
		{
			...
			responses: [
				...
				{ status: 409, description: 'Vehicle not available for selected time slot' }
			],
			examples: [
				{
					title: 'Create a reservation',
					request: 'curl -X POST "https://api.aprofleet.com/v1/reservations" \\\n  -H "Authorization: Bearer YOUR_TOKEN" \\\n  -H "Content-Type: application/json" \\\n  -d \'{"vehicleId":"vehicle-1","customerId":"customer-123","startTime":"2025-10-24T08:00:00Z","endTime":"2025-10-24T12:00:00Z"}\'',
					response: '{\n  "id": "reservation-1",\n  "status": "confirmed"\n}'
				}
			]
		},
		{
			...
			rateLimit: '100 req/min',
			parameters: [
				{ name: 'golfCourseId', type: 'string', required: false, description: 'Filter by golf course' },
				{ name: 'available', type: 'boolean', required: false, description: 'Filter by availability' }
			],
			responses: [
				{
					status: 200,
					...
					example: JSON.stringify({
						data: [
							{ id: 'station-1', name: 'Station A1', status: 'available', power: 7.2, occupied: 2, capacity: 4 }
						]
					}, null, 2)
				}
			],
		...
		},
		{
			...
			rateLimit: '100 req/min',
			parameters: [
				{ name: 'role', type: 'string', required: false, description: 'Filter by role (SUPER_ADMIN, TENANT_OWNER, etc.)' },
			...
			responses: [
				{
					...
					example: JSON.stringify({
						data: [
							{ id: 'user-1', name: 'John Doe', email: 'john@example.com', role: 'TENANT_OWNER', active: true }
						]
					}, null, 2)
				}
			],
		...
		},
		{
			...
			rateLimit: '50 req/min',
			parameters: [
				{ name: 'id', type: 'string', required: true, description: 'Vehicle ID' }
			],
			requestBody: {
				contentType: 'application/json',
				schema: {
		...
		},
		{
			...
			responses: [
				...
				{ status: 409, description: 'Cannot cancel - reservation already started' }
			],
			examples: []
		}
	];
	...
	$: filteredEndpoints = endpoints.filter(endpoint => {
		if (selectedCategory !== 'all' && endpoint.category !== selectedCategory) return false;
		if (selectedMethod !== 'all' && endpoint.method !== selectedMethod) return false;
		if (searchQuery && !endpoint.path.toLowerCase().includes(searchQuery.toLowerCase()) &&
	...
	// Group by category
	$: groupedEndpoints = filteredEndpoints.reduce((acc, endpoint) => {
	...
	function getMethodColor(method: string): string {
		switch (method) {
			case 'GET': return '#1E90FF';
			case 'POST': return '#32CD32';
			case 'PUT': return '#FF8C00';
			case 'PATCH': return '#9370DB';
...
<div class="api-docs-page">
	...
	<div class="auth-section">
		...
		<div class="auth-content">
			...
			<p class="auth-text">
				API tokens can be generated in your account settings. Each token is associated with specific permissions based on your role.
			</p>
		</div>
	</div>
		...
		{#each Object.entries(groupedEndpoints) as [category, categoryEndpoints]}
			<div class="category-group">
				...
				{#each categoryEndpoints as endpoint}
					<div class="endpoint-card" class:expanded={expandedEndpoint === endpoint.id}>
						<div class="endpoint-header" on:click={() => toggleEndpoint(endpoint.id)} on:keydown={(e) => e.key === 'Enter' && toggleEndpoint(endpoint.id)} role="button" tabindex="0">
							<div class="endpoint-row">
								<span class="method-badge" style="color: {getMethodColor(endpoint.method)}">
									{endpoint.method}
								</span>
								...
								<span class="expand-icon">{expandedEndpoint === endpoint.id ? '▼' : '▶'}</span>
							</div>
											...
											<tbody>
												{#each endpoint.parameters as param}
													<tr>
														...
														<td>
															<span class="required-badge" class:yes={param.required}>
								...
								{#if endpoint.requestBody}
									<div class="details-section">
										...
										<div class="schema-section">
											...
											<pre class="code-block">{JSON.stringify(endpoint.requestBody.schema, null, 2)}</pre>
										</div>
										<div class="example-section">
											<div class="example-title">EXAMPLE:</div>
											<pre class="code-block">{endpoint.requestBody.example}</pre>
										</div>
									</div>
								{/if}
									...
									{#each endpoint.responses as response}
										<div class="response-item">
											<div class="response-header">
												<span class="status-code" class:success={response.status < 300} class:error={response.status >= 400}>
													{response.status}
												</span>
										...
										{#each endpoint.examples as example}
											<div class="example-item">
												<div class="example-header">
													...
													<button class="copy-button" on:click={() => copyToClipboard(example.request)}>
														COPY
													</button>
												</div>
	...
	<div class="info-section">
		...
		<div class="info-content">
			<p>API requests are rate-limited to prevent abuse and ensure service stability:</p>
			<ul>
				<li><strong>Read operations (GET):</strong> 100 requests per minute</li>
				<li><strong>Write operations (POST, PUT, PATCH, DELETE):</strong> 50 requests per minute</li>
	...
	<div class="info-section">
		...
		<div class="info-content">
			<table class="error-table">
				...
				<tbody>
					<tr><td>400</td><td>Bad Request - Invalid request format or parameters</td></tr>
					...
					<tr><td>404</td><td>Not Found - Requested resource does not exist</td></tr>
					<tr><td>409</td><td>Conflict - Request conflicts with current state</td></tr>
					...
					<tr><td>503</td><td>Service Unavailable - Temporary service disruption</td></tr>
				</tbody>
			</table>
		</div>
	</div>
