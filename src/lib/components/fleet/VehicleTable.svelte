<script lang="ts">
  ...
  $: displayVehicles =
    vehicles.length > 0 ? vehicles : Array.from($vehicle.vehicles.values());
  ...
  async function sendCommand(vehicleId: string, command: string) {
    ...
    if (result.success) {
      notification.success(
        `COMMAND SENT`,
        `${command.toUpperCase()} sent to vehicle`,
      );
    } else {
  ...
  function exportData() {
    ...
    notification.success(
      "EXPORT COMPLETE",
      `${sortedVehicles.length} vehicles exported`,
    );
  }
</script>
...
<div class="vehicle-table">
  ...
  <div class="table-toolbar">
    <div class="toolbar-left">
      ...
      {#if selectedVehicles.size > 0}
        <div class="bulk-actions">
          ...
          <button
            class="action-button"
            on:click={() => selectedVehicles.clear()}
          >
            CLEAR
          </button>
        </div>
    ...
    <div class="toolbar-right">
      ...
      <button class="action-button" on:click={exportData}> EXPORT CSV </button>
    </div>
  </div>
          ...
          {#if selectable}
            <th class="checkbox-column">
              <input
                type="checkbox"
                on:change={selectAll}
                checked={selectedVehicles.size === paginatedVehicles.length &&
