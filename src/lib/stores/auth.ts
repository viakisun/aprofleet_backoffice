...
function createAuthStore() {
	...
	async function login(email: string, password: string) {
		...
		const demoAccounts: Record<string, { role: Role; name: string; tenantId?: string }> = {
			'admin@dyinnovate.com': { role: Role.SUPER_ADMIN, name: 'DY Admin' },
			'owner@golfcourse.com': { role: Role.TENANT_OWNER, name: 'Golf Course Owner', tenantId: 'tenant-001' },
			'manager@golfcourse.com': { role: Role.TENANT_MANAGER, name: 'Golf Course Manager', tenantId: 'tenant-001' },
			'dev@via.com': { role: Role.SYSTEM_ADMIN, name: 'System Developer' },
			'staff@golfcourse.com': { role: Role.OPERATION_STAFF, name: 'Operation Staff', tenantId: 'tenant-001' }
		};
		...
		if (demoAccounts[email] && password === 'demo123') {
			...
			update(state => ({ ...state, isLoading: false }));
			return { success: false, error: 'Invalid credentials. Use demo accounts with password: demo123' };
		}
	}
	...
	function getPermissionsByRole(role: Role): string[] {
		const permissions: Record<Role, string[]> = {
			[Role.SUPER_ADMIN]: ['*'],
			[Role.TENANT_OWNER]: ['tenant:*', 'vehicle:*', 'reservation:*', 'customer:*'],
			[Role.TENANT_MANAGER]: ['vehicle:*', 'reservation:*', 'customer:read'],
			[Role.SYSTEM_ADMIN]: ['system:*', 'logs:*', 'deployment:*'],
			[Role.OPERATION_STAFF]: ['vehicle:read', 'reservation:manage', 'customer:read']
		};
...
export const isTenantAdmin = derived(
	auth,
	$auth => $auth.user?.role === Role.TENANT_OWNER || $auth.user?.role === Role.TENANT_MANAGER
);
