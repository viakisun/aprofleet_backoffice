...
function createVehicleStore() {
        ...
        if (vehicleIds.length > 0) {
          ...
          if (randomVehicle) {
            ...
            if (randomVehicle.battery.isCharging) {
              randomVehicle.battery.level = Math.min(
                100,
                randomVehicle.battery.level + 1,
              );
            } else if (randomVehicle.status === "operating") {
              randomVehicle.battery.level = Math.max(
                0,
                randomVehicle.battery.level - 1,
              );
            }
  ...
  function updateVehicle(vehicleData: Vehicle) {
    update((state) => {
      ...
      const selected =
        state.selected?.id === vehicleData.id ? vehicleData : state.selected;
  ...
  function updateVehicleLocation(vehicleId: string, location: VehicleLocation) {
    update((state) => {
      ...
      if (vehicle) {
        ...
        // Update selected if it's the same vehicle
        const selected =
          state.selected?.id === vehicleId ? vehicle : state.selected;
  ...
  async function sendCommand(vehicleId: string, command: string, params?: any) {
    ...
    if (
      state.wsConnection &&
      state.wsConnection.readyState === WebSocket.OPEN
    ) {
...
export const vehiclesByStatus = derived(vehicle, ($vehicle) => {
...
export const vehicleStatistics = derived(vehicle, ($vehicle) => {
  ...
  return {
    ...
    avgBattery:
      vehicles.reduce((acc, v) => acc + v.battery.level, 0) / vehicles.length ||
      0,
    lowBattery: vehicles.filter((v) => v.battery.level < 20).length,
    activeReservations: vehicles.filter(
      (v) => v.reservation?.status === "active",
    ).length,
  };
});
