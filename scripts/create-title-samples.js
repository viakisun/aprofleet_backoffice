...
async function createTitlePageSample(lang, fontPath) {
  console.log(`\nCreating title page sample for ${lang.toUpperCase()}...`);
  ...
  // Embed font with subset option for better Korean support
  ...
  console.log(`✓ Title page sample created: ${outputPath}`);
}

async function main() {
  console.log('='.repeat(60));
  console.log('APROFLEET BackOffice - Title Page Samples');
  console.log('='.repeat(60));
  ...
  await createTitlePageSample('en', path.join(FONTS_DIR, 'Roboto-Regular.ttf'));
  await createTitlePageSample('ko', path.join(FONTS_DIR, 'NotoSansKR-Regular.ttf'));
  await createTitlePageSample('jp', path.join(FONTS_DIR, 'NotoSansJP-Regular.ttf'));

  console.log('\n' + '='.repeat(60));
  console.log('✓ All title page samples created successfully!');
  console.log('='.repeat(60));
  console.log(`\nLocation: ${OUTPUT_DIR}\n`);
}
